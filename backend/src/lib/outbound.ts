...
export const dispatchOutbound = async (
  ...
  for (const m of queued ?? []) {
    // Канал push в очередь больше не попадает (миграция 20260926000014):
    // он доставляется самой таблицей notifications — приложение родителя
    // читает её напрямую. Ветка оставлена для записей, накопленных
    // прежней логикой, чтобы очередь не встала на них.
    if (m.channel === "push") {
      await supabaseAdmin
        .from("outbound_messages")
        .update({
          status: "skipped",
          last_error: "Канал push доставляется приложением родителя напрямую; в очереди не нужен.",
        })
        .eq("id", m.id);
      skipped += 1;
      continue;
