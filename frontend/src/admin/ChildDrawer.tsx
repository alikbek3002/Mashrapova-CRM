...
import {
  useChildAccessInfo, useAssignPersonNo, useEnrollFace, useChildAccessEvents,
  useChildFaceStatus, useDeleteFace, useChildAccessWindow, type AccessSubject,
} from "../shared/api/hik";
import {
...
type TabId = "subs" | "att" | "pay" | "deposit" | "freezes" | "notes" | "comments" | "group" | "pt" | "access";
...
export const ChildDrawer = ({
  ...
  const tabs: { id: TabId; ru: string; ky: string; allow: Role[] }[] = [
    ...
    { id: "access",   ru: "Проходная",  ky: "Өткөрмө",      allow: ["admin"] },
  ];
  ...
  return (
    <Modal open={open} onClose={onClose} title={child?.full_name ?? t("Карточка ребёнка", "Бала картасы")} width={760}>
      ...
      <div className="drawer-body">
        ...
        {tab === "access" && role === "admin" && <AccessTab subject={{ kind: "child", id: childId }} lang={lang} />}
      </div>
    </Modal>
  );
};
...
// ============================ Проходная (admin) ============================
// Face ID на турникетах Hikvision: номер персоны, фото лица, журнал проходов.
// Терминал отклоняет фото > 200 КБ, поэтому жмём клиентски до ≤ 190 КБ:
// длинная сторона максимум 1080px, дальше цикл — сначала вниз по качеству
// JPEG (0.85 → 0.45), потом уменьшаем сторону на 20% и повторяем.
// Терминалу хватает лица ≥80×80; 720px/≤120КБ качается по 4G вдвое быстрее, чем 1080/190
const FACE_MAX_BYTES = 120 * 1024;

type CompressedPhoto = { base64: string; dataUrl: string; bytes: number };

const compressFacePhoto = (file: File): Promise<CompressedPhoto> =>
  new Promise((resolve, reject) => {
    ...
    img.onload = () => {
      ...
      if (longSide < 160) { reject(new Error("photo_too_small")); return; }
      let side = Math.min(720, longSide);
      ...
      while (side >= 300) {
        ...
        for (let q = 0.85; q >= 0.44; q -= 0.1) {
          const dataUrl = canvas.toDataURL("image/jpeg", q);
          const base64 = dataUrl.slice(dataUrl.indexOf(",") + 1);
          const bytes = Math.floor(base64.length * 0.75);
          last = { base64, dataUrl, bytes };
          if (bytes <= FACE_MAX_BYTES) { resolve(last); return; }
        }
...
const ACCESS_EVENT_LBL: Record<string, { ru: string; ky: string; color: string }> = {
  face_ok:   { ru: "Лицо ✓",       ky: "Жүз ✓",         color: "var(--green)" },
  face_fail: { ru: "Не распознан", ky: "Таанылган жок", color: "var(--red-600)" },
  card_ok:   { ru: "Карта",        ky: "Карта",         color: "var(--ink)" },
  other:     { ru: "Другое",       ky: "Башка",         color: "var(--muted)" },
};

// Съёмка с камеры (getUserMedia): выбор устройства (вебкамера/встроенная),
// живое превью и снимок. Превью зеркалим только визуально — на терминал
// фото уходит в реальной ориентации.
const CameraCapture = ({ lang, onCapture, onClose }: {
  lang: Lang;
  ...
  const startStream = async (id?: string) => {
    ...
    try {
      ...
      // названия камер браузер отдаёт только после выданного разрешения
      ...
      setCamErr(
        e instanceof DOMException && (e.name === "NotAllowedError" || e.name === "SecurityError")
          ? t("Доступ к камере запрещён — разрешите его в настройках браузера (значок 🔒 в адресной строке)",
              "Камерага уруксат жок — браузер жөндөөлөрүнөн уруксат бериңиз")
          : t("Камера недоступна", "Камера жеткиликсиз"),
      );
    }
  };
  ...
  const snap = () => {
    ...
    canvas.toBlob((b) => {
      if (!b) return;
      stopStream();
      onCapture(new File([b], "camera.jpg", { type: "image/jpeg" }));
    }, "image/jpeg", 0.92);
  };
  ...
  return (
    <div style={{
      ...
      {camErr ? (
        <>
          <div className="field__error">{camErr}</div>
          <button className="btn" style={{ marginTop: 8 }} onClick={onClose}>{t("Закрыть", "Жабуу")}</button>
        </>
      ) : (
        <>
          {devices.length > 1 && (
            <select
              value={deviceId}
              onChange={(e) => { setDeviceId(e.target.value); startStream(e.target.value); }}
              style={{
                marginBottom: 8, padding: "6px 10px", fontSize: 13,
                border: "1px solid var(--line)", borderRadius: "var(--r-xs)",
                background: "var(--surface)", color: "var(--ink)", maxWidth: "100%",
              }}
            >
              {devices.map((d, i) => (
                <option key={d.deviceId} value={d.deviceId}>
                  {d.label || t(`Камера ${i + 1}`, `Камера ${i + 1}`)}
                </option>
              ))}
            </select>
          )}
          <video
            ...
            style={{
              ...
              borderRadius: "var(--r-xs)", border: "1px solid var(--line)",
              background: "#000", transform: "scaleX(-1)",
            }}
          />
          <div style={{ display: "flex", gap: 8, marginTop: 10 }}>
            <button className="btn btn--primary" onClick={snap}>
              {t("Снимок", "Сүрөткө тартуу")}
            </button>
            <button className="btn" onClick={() => { stopStream(); onClose(); }}>
              {t("Отмена", "Жокко чыгаруу")}
            </button>
          </div>
        </>
...
// Вкладка «Проходная» — общая для карточки ребёнка и карточки сотрудника.
export const AccessTab = ({ subject, lang }: { subject: AccessSubject; lang: Lang }) => {
  const t = (ru: string, ky: string) => (lang === "ru" ? ru : ky);
  const locale = lang === "ru" ? "ru-RU" : "ky-KG";
  const { data: info, isLoading } = useChildAccessInfo(subject);
  const { data: events = [] } = useChildAccessEvents(subject);
  const assign = useAssignPersonNo();
  const enroll = useEnrollFace();
  const faceStatus = useChildFaceStatus(subject, !!info?.access_person_no);
  const deleteFace = useDeleteFace();
  // окно доступа по расписанию — только у детей (сотрудники без ограничений)
  const accessWindow = useChildAccessWindow(subject.kind === "child" && info?.access_person_no ? subject.id : null);
  ...
  const processFile = async (f: File) => {
    ...
    setCompressing(true);
    try {
      setPhoto(await compressFacePhoto(f));
    } catch {
      setPhoto(null);
      setPhotoErr(t("Не удалось обработать фото. Выберите другой файл.", "Сүрөттү иштетүү мүмкүн болгон жок. Башка файл тандаңыз."));
    } finally {
  ...
  const onCameraShot = async (f: File) => {
    setCamOpen(false);
    await processFile(f);
  };

  const send = () => {
    if (!photo || !info?.access_person_no) return;
    enroll.mutate(
      { subject, photoBase64: photo.base64 },
  ...
  const onDeleteFace = () => {
    if (!confirm(t(
      ...
      "Баланын жүзүн жана карточкасын терминалдан өчүрөсүзбү?",
    ))) return;
    deleteFace.mutate(subject);
  };

  if (isLoading) {
    return <div className="empty"><div className="empty__title">{t("Загрузка…", "Жүктөлүүдө…")}</div></div>;
  }

  const dirLabel = (d: string) =>
    d === "in" ? t("Вход", "Кирүү") : d === "out" ? t("Выход", "Чыгуу") : "—";

  return (
    <>
      {/* --- Номер на проходной --- */}
      <div className="m-sect">
        <span className="m-sect__title">{t("Номер на проходной", "Өткөрмөдөгү номер")}</span>
      </div>
      {info?.access_person_no ? (
        <div style={{
          padding: "12px 14px", marginTop: 8,
          background: "var(--bg-soft)", border: "1px solid var(--line)",
          borderRadius: "var(--r-sm)",
        }}>
          <div style={{ fontSize: 11, color: "var(--muted)" }}>{t("Номер в терминале", "Терминалдагы номер")}</div>
          <div style={{ fontSize: 30, fontWeight: 700, fontFamily: "var(--font-mono)", letterSpacing: 1 }}>
            {info.access_person_no}
          </div>
        </div>
      ) : (
        <div style={{
          ...
          <div style={{ flex: 1, fontSize: 13, color: "var(--muted)" }}>
            {t("Номер не присвоен", "Номер ыйгарылган эмес")}
          </div>
          <button
            className="btn btn--primary"
            onClick={() => assign.mutate(subject)}
            disabled={assign.isPending}
          >
            <Icon name="plus" size={14} /> {assign.isPending ? "…" : t("Присвоить номер", "Номер ыйгаруу")}
          </button>
          {assign.error && (
            <div className="field__error" style={{ width: "100%" }}>{(assign.error as Error).message}</div>
          )}
        </div>
      )}

      {/* --- Доступ сегодня (режим «по расписанию») --- */}
      {subject.kind === "child" && info?.access_person_no && accessWindow.data && (() => {
        const w = accessWindow.data;
        const hm = (iso: string) => new Date(iso).toLocaleTimeString(locale, { hour: "2-digit", minute: "2-digit" });
        const pushedOk = w.grants.filter((g) => !g.error && g.state === w.desired.state).length;
        return (
          <>
            <div className="m-sect" style={{ paddingTop: 14 }}>
              <span className="m-sect__title">{t("Доступ сегодня", "Бүгүнкү кирүү")}</span>
            </div>
            <div style={{ fontSize: 13, marginTop: 8, lineHeight: 1.5 }}>
              {!w.settings.enabled ? (
                <span style={{ color: "var(--muted)" }}>
                  {t("Режим «по расписанию» выключен — доступ бессрочный", "«Расписание боюнча» режими өчүк — кирүү мөөнөтсүз")}
                </span>
              ) : w.intervals.length === 0 ? (
                <span style={{ color: "var(--muted)" }}>
                  {t("Занятий сегодня нет — турникет не пропустит", "Бүгүн сабак жок — турникет өткөрбөйт")}
                </span>
              ) : (
                <>
                  {w.intervals.map((i) => (
                    <div key={i.from}>
                      <span style={{ fontFamily: "var(--font-mono)", fontWeight: 600 }}>{hm(i.from)}–{hm(i.until)}</span>
                      {w.desired.state === "window" && w.desired.from === i.from && (
                        <span style={{ color: "var(--green)", marginLeft: 8 }}>
                          {t("← выставлено на терминале", "← терминалда коюлган")}
                          {w.grants.length > 0 && ` (${pushedOk}/${w.grants.length})`}
                        </span>
                      )}
                    </div>
                  ))}
                  {w.desired.state === "none" && (
                    <div style={{ color: "var(--muted)" }}>{t("Занятия на сегодня закончились", "Бүгүнкү сабактар бүттү")}</div>
                  )}
                </>
              )}
              {w.inside && (
                <div style={{ marginTop: 4, color: "var(--blue-700, var(--ink))", fontWeight: 600 }}>
                  {t("В здании с", "Имаратта")} {hm(w.inside.entered_at)}{lang === "ky" ? t("", "дан бери") : ""}
                </div>
              )}
              {w.grants.some((g) => g.error) && (
                <div style={{ marginTop: 4, fontSize: 12, color: "var(--red-600)" }}>
                  {t("Ошибка отправки на терминал", "Терминалга жөнөтүү катасы")}: {w.grants.find((g) => g.error)?.error}
                </div>
              )}
            </div>
          </>
      ...
      <div className="m-sect" style={{ paddingTop: 14 }}>
        <span className="m-sect__title">{t("Лицо для распознавания", "Таануу үчүн жүз")}</span>
      </div>
      <div style={{ fontSize: 13, marginTop: 8, display: "flex", alignItems: "center", gap: 10, flexWrap: "wrap" }}>
        {!info?.access_person_no ? (
          <span style={{ color: "var(--muted)" }}>{t("Лицо не заведено", "Жүз киргизилген эмес")}</span>
        ) : faceStatus.isLoading ? (
          <span style={{ color: "var(--muted)" }}>{t("Проверяю на терминале…", "Терминалда текшерилүүдө…")}</span>
        ) : faceStatus.isError ? (
          <span style={{ color: "var(--yellow-ink)", fontWeight: 600 }}>
            {t("Терминал не в сети — статус неизвестен", "Терминал байланышта эмес — абалы белгисиз")}
            {info?.face_enrolled_at && (
              <span style={{ color: "var(--muted)", fontWeight: 400 }}>
                {" "}({t("последняя заливка", "акыркы жүктөө")}: {new Date(info.face_enrolled_at).toLocaleDateString(locale)})
              </span>
            )}
          </span>
        ) : faceStatus.data ? (
          <>
            <span style={{ color: "var(--green)", fontWeight: 600 }}>
              {t("Лицо на терминале ✅", "Жүз терминалда ✅")}
            </span>
            <button className="btn btn--warn" onClick={onDeleteFace} disabled={deleteFace.isPending}>
              {deleteFace.isPending ? t("Удаляю…", "Өчүрүлүүдө…") : t("Удалить с терминала", "Терминалдан өчүрүү")}
            </button>
          </>
        ) : (
          <span style={{ color: "var(--muted)" }}>{t("Лицо не заведено", "Жүз киргизилген эмес")}</span>
      ...
      {deleteFace.error && (
        <div className="field__error" style={{ marginTop: 6 }}>
          {(() => {
            const msg = (deleteFace.error as Error).message;
            if (msg.includes("isup_unreachable")) {
              return t("Терминал не в сети — не удалось удалить. Попробуйте позже.",
                "Терминал байланышта эмес — өчүрүү мүмкүн болгон жок. Кийинчерээк аракет кылыңыз.");
            }
            return `${t("Не удалось удалить", "Өчүрүү мүмкүн болгон жок")}: ${msg}`;
          })()}
        </div>
      )}
      {/* Фото с турникета — оно же аватарка в системе (журнал, карточка,
          списки), поэтому показываем, что именно сейчас залито. */}
      {info?.face_photo_path && (
        <div style={{ marginTop: 10, display: "flex", gap: 12, alignItems: "center", flexWrap: "wrap" }}>
          <img
            src={resolveStorageUrl(info.face_photo_path) ?? ""}
            alt={t("Фото на терминале", "Терминалдагы сүрөт")}
            style={{
          ...
          />
          <div style={{ fontSize: 11.5, color: "var(--muted)", maxWidth: 260, lineHeight: 1.45 }}>
            {t(
              "Это же фото показывается в системе: в журнале, карточке и списках.",
              "Ушул эле сүрөт системада көрүнөт: журналда, картада жана тизмелерде.",
            )}
          </div>
        </div>
      )}
      <div style={{ fontSize: 11.5, color: "var(--muted)", marginTop: 4 }}>
        {t(
          "Одно фронтальное лицо, без очков и головного убора. Фото сжимается до 190 КБ автоматически.",
          "Бир фронталдык жүз, көз айнексиз жана баш кийимсиз. Сүрөт автоматтык түрдө 190 КБга чейин кысылат.",
        )}
      </div>
      <input ref={fileRef} type="file" accept="image/*" style={{ display: "none" }} onChange={onFile} />
      <div style={{ display: "flex", gap: 8, alignItems: "center", marginTop: 10, flexWrap: "wrap" }}>
        <button className="btn btn--primary" onClick={() => setCamOpen((v) => !v)} disabled={compressing}>
          {camOpen ? t("Закрыть камеру", "Камераны жабуу") : t("📷 Сфотографировать", "📷 Сүрөткө тартуу")}
        </button>
        <button className="btn" onClick={() => fileRef.current?.click()} disabled={compressing}>
          {compressing ? t("Сжимаю…", "Кысылууда…") : t("Выбрать файл", "Файл тандоо")}
        </button>
      </div>
      {camOpen && <CameraCapture lang={lang} onCapture={onCameraShot} onClose={() => setCamOpen(false)} />}
      {photoErr && <div className="field__error" style={{ marginTop: 8 }}>{photoErr}</div>}
      {photo && (
        <div style={{
          marginTop: 10, padding: 12,
          background: "var(--bg-soft)", border: "1px solid var(--line)",
          ...
          <img
            src={photo.dataUrl}
            alt={t("Превью фото", "Сүрөттүн превьюсу")}
            style={{ width: 96, height: 96, objectFit: "cover", borderRadius: "var(--r-xs)", border: "1px solid var(--line)" }}
          />
          <div style={{ flex: 1, minWidth: 180 }}>
            <div style={{ fontSize: 12, color: "var(--muted)", marginBottom: 8 }}>
              JPEG · {Math.round(photo.bytes / 1024)} {t("КБ", "КБ")}
            </div>
            <button
              className="btn btn--primary"
              onClick={send}
              disabled={!info?.access_person_no || enroll.isPending}
              title={!info?.access_person_no ? t("Сначала присвойте номер", "Адегенде номер ыйгарыңыз") : undefined}
            >
              <Icon name="check" size={14} stroke={2.5} />{" "}
              {enroll.isPending ? t("Отправляю…", "Жөнөтүлүүдө…") : t("Отправить на терминал", "Терминалга жөнөтүү")}
            </button>
            {!info?.access_person_no && (
              <div style={{ fontSize: 11.5, color: "var(--muted)", marginTop: 6 }}>
                {t("Сначала присвойте номер", "Адегенде номер ыйгарыңыз")}
              </div>
            )}
          </div>
        </div>
      )}
      {enroll.isSuccess && !photo && (
        <div style={{
          marginTop: 10, padding: "10px 12px", fontSize: 13, fontWeight: 600,
          background: "var(--green-50)", color: "var(--green)",
          border: "1px solid oklch(0.85 0.16 150)", borderRadius: "var(--r-sm)",
        }}>
          {t("Фото принято терминалом ✅", "Сүрөттү терминал кабыл алды ✅")}
        </div>
      )}
      {enroll.error && (
        <div className="field__error" style={{ marginTop: 8 }}>
          {(() => {
            const msg = (enroll.error as Error).message;
            if (msg.includes("isup_unreachable")) {
              return t("Терминал не в сети — фото не отправлено. Попробуйте позже.",
                "Терминал байланышта эмес — сүрөт жөнөтүлгөн жок. Кийинчерээк аракет кылыңыз.");
            }
            if (msg.includes("no_person_no")) {
              return t("Сначала присвойте номер на проходной", "Адегенде өткөрмө номерин ыйгарыңыз");
            }
            return `${t("Терминал отклонил фото", "Терминал сүрөттү кабыл алган жок")}: ${msg} ${t("(нужно одно фронтальное лицо)", "(бир фронталдык жүз керек)")}`;
          })()}
        </div>
      )}
      ...
      {/* --- История проходов --- */}
      <div className="m-sect" style={{ paddingTop: 14 }}>
        <span className="m-sect__title">{t("История проходов", "Өтүүлөр тарыхы")}</span>
      </div>
      {events.length === 0 ? (
        <div className="empty"><div className="empty__title">{t("Проходов нет", "Өтүүлөр жок")}</div></div>
      ) : (
        <div style={{ display: "flex", flexDirection: "column", gap: 4, marginTop: 8 }}>
          {events.map((e) => {
            const lbl = ACCESS_EVENT_LBL[e.event_type] ?? ACCESS_EVENT_LBL.other!;
            return (
              <div
                key={e.id}
                style={{
              ...
              >
                ...
                <span style={{ color: lbl.color, fontWeight: 600, flex: 1 }}>{lbl[lang]}</span>
                <span style={{ color: "var(--muted)" }}>{dirLabel(e.direction)}</span>
              </div>
            );
          })}
        </div>
