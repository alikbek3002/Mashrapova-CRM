...
//
// Задачи считаются здесь, а не читаются из notifications: инбокса у
// офиса пока нет (таблицу notifications читает только приложение
// родителя), а по ТЗ §8.3 менеджер должен видеть просрочку сразу.
// refresh_lead_sla() в БД параллельно проставляет отметки и пишет
// уведомления — это журнал и будущая рассылка, а не источник для UI.
